...
class SimulatorSettingPair:
    ...
    def skip(self) -> bool:
        ...
        return self.mode == SimulatorTestMode.DISABLED or (
            self.mode == SimulatorTestMode.IF_AVAILABLE
            and not self.simulator.available
        )
...
def pytest_addoption(parser):
    parser.addoption(
        ...
        help="Simulation test mode. One of {[m.value for m in SimulatorTestMode]}.",
    )
    for s in simulators.keys():
        parser.addoption(
            ...
            action="store",
            default=None,
            help=f"{s} test mode. One of {[m.value for m in SimulatorTestMode]}.",
        )
...
def pytest_collection_modifyitems(config, items):
    ...
    simulator_settings: Dict[str, SimSettingPair] = {s.marker: default_mode for s in simulators.values()}
