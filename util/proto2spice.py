...
def define_options(optparser: optparse.OptionParser):
    optparser.add_option('-l', '--library', dest='library', default=None,
                         help='path to vlsir.raw.Library, in either text- or'
                              'binary-format (will try binary first)')
    optparser.add_option('-p', '--package', dest='package', default=None,
                         help='path to vlsir.circuit.Package, in either text-'
                              ' or binary-format proto.')
    optparser.add_option('-o', '--output', dest='output', default='output.sp',
                         help='destination path of spice netlist')
    optparser.add_option('-f', '--format', dest='format', default='xyce',
...
def load_package(path: str) -> vlsir_circuit.Package:
    ...
    try:
        ...
        try:
            with open(path, 'r') as f:
                package_pb = text_format.Parse(
                    f.read(), vlsir_circuit.Package())
        except Exception as err:
...
def process(options: optparse.Values):
    ...
    with open(options.output, 'w') as output:
        tools.netlist(
            package_pb, output, fmt=options.format, opts=netlist_options)
